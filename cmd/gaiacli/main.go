...
import (
	...
	authxcmd "github.com/SoftWorxDevelopments/mypos-sdk/modules/authx/client/cli"
	bankxcmd "github.com/SoftWorxDevelopments/mypos-sdk/modules/bankx/client/cli"
	distrxcmd "github.com/SoftWorxDevelopments/mypos-sdk/modules/distributionx/client/cli"
...
func fixDescriptions(cmd *cobra.Command) {
	// cosmosvalconspubXXX -> myposvalconspubXXX
	if idx := strings.Index(cmd.Long, "cosmosvalconspub"); idx >= 0 {
		...
		myposchainPubKey := sdk.MustBech32ifyConsPub(pubKey)
		cmd.Long = strings.Replace(cmd.Long, cosmosPubKey, myposchainPubKey, -1)
		//fmt.Printf("%s -> %s\n", cosmosPubKey, myposPubKey)
	}
	// cosmosvaloperXXX -> myposvaloperXXX
	for {
		if idx := strings.Index(cmd.Long, "cosmosvaloper"); idx >= 0 {
			...
			myposchainValAddr := sdk.ValAddress(rawValAddr).String()
			cmd.Long = strings.Replace(cmd.Long, cosmosValAddr, myposchainValAddr, -1)
			//fmt.Printf("%s -> %s\n", cosmosValAddr, myposchainValAddr)
		} else {
			break
		}
	}
	...
	if idx := strings.Index(cmd.Long, "cosmos1..."); idx >= 0 {
		cmd.Long = strings.ReplaceAll(cmd.Long, "cosmos1...", "mypos1...")
	}
	if idx := strings.Index(cmd.Long, "cosmos1"); idx >= 0 {
		cosmosAccAddr := cmd.Long[idx : idx+45]
		rawAccAddr := rawAddressFromBech32("cosmos", cosmosAccAddr)
		myposchainAccAddr := sdk.AccAddress(rawAccAddr).String()
		cmd.Long = strings.Replace(cmd.Long, cosmosAccAddr, myposchainAccAddr, -1)
		//fmt.Printf("%s -> %s\n", cosmosAccAddr, myposchainAccAddr)
	}
	if idx := strings.Index(cmd.Long, "0stake"); idx >= 0 {
		cmd.Long = strings.Replace(cmd.Long, "0stake", "0mypc", -1)
		//fmt.Printf("%s -> %s\n", "stake", myposchain.MYPC)
	}

	// uatom -> mypc
	for _, flagName := range []string{client.FlagFees, client.FlagGasPrices} {
		if flag := cmd.Flag(flagName); flag != nil {
			flag.Usage = strings.Replace(flag.Usage, "uatom", myposchain.MYPC, -1)
			//fmt.Printf("%s -> %s\n", "uatom", myposchain.MYPC)
		}
	}

	if cmd.Name() == "submit-proposal" {
		cmd.Long = strings.ReplaceAll(cmd.Long, `10test`, `10mypc`)
	}
	if cmd.Name() == "param-change" {
		cmd.Long = strings.ReplaceAll(cmd.Long, `"stake"`, `"mypc"`)
	}
	if cmd.Name() == "community-pool-spend" {
		cmd.Long = strings.ReplaceAll(cmd.Long, `"stake"`, `"mypc"`)
		cmd.Long = strings.ReplaceAll(cmd.Long, "Pay me some Atoms!", "Pay me some MYPCs!")
	}
